#include <gbdk/platform.h>
#include <stdint.h>

#include "screen.h"
#include "systemdetect.h"

BANKREF(module_screen)

const uint8_t * const screen_tile_addresses[DEVICE_SCREEN_HEIGHT] = {
    TO_TILE_ADDRESS(TILE_BANK_0, 0x00),
    TO_TILE_ADDRESS(TILE_BANK_0, 0x20),
    TO_TILE_ADDRESS(TILE_BANK_0, 0x40),
    TO_TILE_ADDRESS(TILE_BANK_0, 0x60),
    TO_TILE_ADDRESS(TILE_BANK_0, 0x80),
    TO_TILE_ADDRESS(TILE_BANK_0, 0xA0),
    TO_TILE_ADDRESS(TILE_BANK_0, 0xC0),
    TO_TILE_ADDRESS(TILE_BANK_0, 0xE0),
    TO_TILE_ADDRESS(TILE_BANK_1, 0x00),
    TO_TILE_ADDRESS(TILE_BANK_1, 0x20),
    TO_TILE_ADDRESS(TILE_BANK_1, 0x40),
    TO_TILE_ADDRESS(TILE_BANK_1, 0x60),
    TO_TILE_ADDRESS(TILE_BANK_1, 0x80),
    TO_TILE_ADDRESS(TILE_BANK_1, 0xA0),
    TO_TILE_ADDRESS(TILE_BANK_1, 0xC0),
    TO_TILE_ADDRESS(TILE_BANK_1, 0xE0),
    TO_TILE_ADDRESS(TILE_BANK_2, 0x00),
    TO_TILE_ADDRESS(TILE_BANK_2, 0x20),
    TO_TILE_ADDRESS(TILE_BANK_2, 0x40),
    TO_TILE_ADDRESS(TILE_BANK_2, 0x60),
    TO_TILE_ADDRESS(TILE_BANK_2, 0x80),
    TO_TILE_ADDRESS(TILE_BANK_2, 0xA0),
    TO_TILE_ADDRESS(TILE_BANK_2, 0xC0),
    TO_TILE_ADDRESS(TILE_BANK_2, 0xE0)
};

const uint8_t screen_tile_map[DEVICE_SCREEN_HEIGHT * DEVICE_SCREEN_WIDTH] = {
    
};

uint8_t INIT_module_screen(void) BANKED {
    screen_clear_rect(0, 0, DEVICE_SCREEN_WIDTH, DEVICE_SCREEN_HEIGHT, WHITE_ON_BLACK);
    return 0;
}
